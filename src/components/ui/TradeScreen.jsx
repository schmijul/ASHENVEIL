...
export default function TradeScreen() {
  const { tradeOpen, activeMerchantId } = useGameStore((state) => state.ui);
  const gold = useGameStore((state) => state.player.gold);
  const closeTrade = useGameStore((state) => state.closeTrade);
  const { inventory, getItemDefinition } = useInventoryStore((state) => ({
    inventory: state.inventory,
    getItemDefinition: state.getItemDefinition,
  }));
  const { getMerchantInventory, buyItem, sellItem } = useTradeStore((state) => ({
    getMerchantInventory: state.getMerchantInventory,
